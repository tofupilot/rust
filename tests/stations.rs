...
async fn get_station_returns_matching_data() {
    ...
    let fetched = client().stations().get().id(&id).send().await.unwrap();
...
async fn list_stations_returns_list() {
    ...
    let result = client().stations().list().send().await.unwrap();
...
async fn remove_station_returns_id() {
    ...
    let removed = client().stations().remove().id(&id).send().await.unwrap();
...
async fn remove_station_twice_fails() {
    let (id, _) = create_station(&uid()).await;
    client().stations().remove().id(&id).send().await.unwrap();

    let result = client().stations().remove().id(&id).send().await;
    assert!(matches!(result, Err(tofupilot::Error::NotFound(_))));
}
...
async fn get_current_with_user_key_returns_forbidden() {
    let result = client().stations().get_current().send().await;
