...
async fn get_part_returns_matching_data() {
    ...
    let fetched = client().parts().get().number(&number).send().await.unwrap();
...
async fn list_parts_returns_list() {
    ...
    let result = client().parts().list().send().await.unwrap();
...
async fn update_part_number() {
    ...
    // Old number 404
    let result = client().parts().get().number(&old_number).send().await;
    assert!(matches!(result, Err(tofupilot::Error::NotFound(_))));
}
