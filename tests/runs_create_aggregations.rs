...
async fn aggregation_type_avg() {
    let u = uid();
    let id = create_with_aggregation(
        &u,
        "test_avg",
        50.0,
    ...
    let aggs = fetched.phases.unwrap()[0].measurements[0]
        .aggregations
        .clone();
    if let NullableField::Value(a) = aggs {
        assert_eq!(1, a.len());
        assert!(a[0].r#type.eq_ignore_ascii_case("avg"));
    } else {
...
async fn aggregation_type_min() {
    ...
    let aggs = fetched.phases.unwrap()[0].measurements[0]
        ...
        .clone();
    if let NullableField::Value(a) = aggs {
        assert!(a[0].r#type.eq_ignore_ascii_case("min"));
    } else {
        panic!("expected aggregations");
    }
}
...
async fn aggregation_type_max() {
    ...
    let aggs = fetched.phases.unwrap()[0].measurements[0]
        ...
        .clone();
    if let NullableField::Value(a) = aggs {
        assert!(a[0].r#type.eq_ignore_ascii_case("max"));
    } else {
        panic!("expected aggregations");
    }
}
...
async fn aggregation_type_sum() {
    ...
    let aggs = fetched.phases.unwrap()[0].measurements[0]
        ...
        .clone();
    if let NullableField::Value(a) = aggs {
        assert!(a[0].r#type.eq_ignore_ascii_case("sum"));
    } else {
        panic!("expected aggregations");
    }
}
...
async fn aggregation_type_count() {
    ...
    let aggs = fetched.phases.unwrap()[0].measurements[0]
        ...
        .clone();
    if let NullableField::Value(a) = aggs {
        assert!(a[0].r#type.eq_ignore_ascii_case("count"));
    } else {
        panic!("expected aggregations");
    }
}
...
async fn aggregation_type_std() {
    ...
    let aggs = fetched.phases.unwrap()[0].measurements[0]
        ...
        .clone();
    if let NullableField::Value(a) = aggs {
        assert!(a[0].r#type.eq_ignore_ascii_case("std"));
    } else {
        panic!("expected aggregations");
    }
}
...
async fn aggregation_type_median() {
    ...
    let aggs = fetched.phases.unwrap()[0].measurements[0]
        ...
        .clone();
    if let NullableField::Value(a) = aggs {
        assert!(a[0].r#type.eq_ignore_ascii_case("median"));
    } else {
        panic!("expected aggregations");
    }
}
...
async fn aggregation_with_validators() {
    ...
    let aggs = fetched.phases.unwrap()[0].measurements[0]
        ...
        .clone();
    if let NullableField::Value(a) = aggs {
        if let NullableField::Value(v) = &a[0].validators {
            assert_eq!(2, v.len());
        } else {
            panic!("expected validators on aggregation");
        }
    } else {
        panic!("expected aggregations");
    }
...
async fn aggregation_with_special_char_type() {
    ...
    let aggs = fetched.phases.unwrap()[0].measurements[0]
        ...
        .clone();
    if let NullableField::Value(a) = aggs {
        assert!(a[0].r#type.eq_ignore_ascii_case("percentile_95"));
    } else {
        panic!("expected aggregations");
    }
}
