...
async fn get_run_includes_phases_and_measurements() {
    ...
    let init = phases
        .iter()
        .find(|p| p.name == "init_phase")
        .expect("init_phase");
    assert_eq!(2, init.measurements.len());

    let stress = phases
        .iter()
        .find(|p| p.name == "stress_phase")
        .expect("stress_phase");
...
async fn get_run_includes_logs() {
    ...
    assert!(logs
        .iter()
        .any(|l| l.message == "Initializing device" && l.source_file == "device.rs"));
    assert!(logs
        .iter()
        .any(|l| l.message == "Recovered from transient fault" && l.line_number == 55));
}
